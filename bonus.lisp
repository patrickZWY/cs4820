...
#|
 ...
 Q4 (final tasks):
 1) Ordered permutations are equal.
 2) qsort/isort return ordered permutations of input.

|#

; If an element is <= the head of an ordered list, insert puts it in front.
(property insert-front-when-<= (a :all l :tl)
          :h (^ (orderedp l)
                (or (endp l)
                    (<<= a (first l))))
          :b (== (insert a l)
...
; isort is identity on ordered lists.
(property isort-ordered-id (l :tl)
          :h (orderedp l)
          :b (== (isort l) l))

; We use sorted-normal-form equivalence as permutation.
(definec perm<<= (x :tl y :tl) :bool
  (== (isort x) (isort y)))

; Ordered lists that are permutations of each other are equal.
(property ordered-perm<<=-equal (x :tl y :tl)
          :h (^ (orderedp x)
                (orderedp y)
                (perm<<= x y))
          :b (== x y)
...
; isort returns an ordered permutation of its input.
(property isort-ordered-perm<<= (x :tl)
          (^ (orderedp (isort x))
             (perm<<= (isort x) x)))

; qsort returns an ordered permutation of its input.
